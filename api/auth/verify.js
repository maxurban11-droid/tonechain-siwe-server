...
// WICHTIG: .js belassen. Läuft auf Node (kein Edge nötig).
...
export default async function handler(req, res) {
  ...
  // Echo-Origin (wie in deiner funktionierenden alten Datei); wir blocken ggf. später per 403,
  // aber Preflight bekommt IMMER CORS-Header und credentials erlaubt:
  ...
  if (req.method === "OPTIONS") {
    // Preflight: sofort beenden, kein weiterer Code/Import
  ...
  // === Ab hier ist Preflight schon erledigt; jetzt dürfen wir strenger sein ===
  try {
    // Origin-Whitelist
    let allowed = false;
    try { allowed = !!(origin && ALLOWED_DOMAINS.has(new URL(origin).hostname)); } catch {}
    if (!allowed) {
      // Hinweis: CORS-Header sind gesetzt, Browser sieht saubere Antwort
    ...
    // Ethers verify (dynamisch importieren → Preflight ist davon unabhängig)
    ...
    // Supabase Admin-Client (dynamisch importieren, um Top-Level-Init zu vermeiden)
    ...
    // user_id bestimmen (optional, für Session)
    let userId = null;
    ...
    // CORS-Header sind schon gesetzt
    return res.status(500).json({ ok: false, code: "INTERNAL_ERROR" });
  }
}

// (optional, falls deine Plattform es unterstützt/benötigt)

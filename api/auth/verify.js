// /api/auth/verify.js — stabile SIWE-Verify-Route mit eigenem CORS (Node runtime)

import crypto from "node:crypto";
import { createClient } from "@supabase/supabase-js";

/* ============ CORS: immer zuerst, robust & eigenständig ============ */
function setCors(req, res) {
  const origin = req.headers.origin || "";
  // Echo den Origin (Framer Sandbox braucht das), sonst * für lokale Tests
  ...
  res.setHeader(
    ...
    "Content-Type, Authorization, X-Requested-With, X-SIWE-Version"
  );
}
...
export default async function handler(req, res) {
  ...
  if (req.method === "OPTIONS") {
    // Preflight ok – keine weitere Logik!
    res.status(204).end();
    return;
  }
  if (req.method !== "POST") {
  ...
  // ---- Policy-Gates (jetzt NACH CORS, damit Browser nie blockt) ----
  if (!originAllowed()) {
  ...
  // Nonce aus Cookie
  const cookieNonce = getCookie(COOKIE_NONCE);
  if (!cookieNonce) {
  ...
  // user_id holen (optional für Session)
  let userId = null;
  ...
  try {
    ...
    clearCookie(COOKIE_NONCE);
    setCookie(COOKIE_SESSION, sessionValue, { maxAgeSec: SESSION_TTL_SEC });
